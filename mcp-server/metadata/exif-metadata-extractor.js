...
class ExifMetadataExtractor {
    ...
    async extract(image) {
        try {
            ...
            throw new errors_1.MetadataExtractionError(`Failed to extract metadata: ${error.message}`, error);
        }
    }
    /**
    ...
    parseExifData(exifBuffer) {
        try {
            ...
            const exif = (0, exif_reader_1.default)(exifBuffer);
            if (!exif) {
                return undefined;
            }
            const exifData = {};
    ...
    parseGPSData(gpsData) {
        ...
        if (gpsData.GPSLatitude && gpsData.GPSLatitudeRef) {
            const lat = this.convertDMSToDecimal(gpsData.GPSLatitude[0], gpsData.GPSLatitude[1], gpsData.GPSLatitude[2]);
            gps.latitude = gpsData.GPSLatitudeRef === 'S' ? -lat : lat;
        }
        // Parse longitude
        if (gpsData.GPSLongitude && gpsData.GPSLongitudeRef) {
            const lon = this.convertDMSToDecimal(gpsData.GPSLongitude[0], gpsData.GPSLongitude[1], gpsData.GPSLongitude[2]);
...
//# sourceMappingURL=exif-metadata-extractor.js.map
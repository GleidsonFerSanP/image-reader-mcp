 ...
 */
export const batchProcessImagesTool: IMCPTool = {
...
Note: Processing is sequential to manage memory usage. Maximum 10 images per batch.`,
  ...
  handler: async (args: unknown): Promise<MCPToolResponse> => {
    try {
      const { sources, operation, options = {} } = args as BatchProcessArgs;
      ...
      return {
        content: [
          {
            type: 'text',
            text: `Error in batch processing: ${(error as Error).message}`
          }
        ],
        isError: true
      };

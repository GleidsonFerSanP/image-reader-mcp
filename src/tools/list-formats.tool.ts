...
import { SUPPORTED_FORMATS } from '../utils/format-detector';

/**
 * MCP Tool: list_supported_formats
 * Lists all supported image formats with their capabilities
 */
export const listSupportedFormatsTool: IMCPTool = {
  ...
  handler: async (): Promise<MCPToolResponse> => {
    try {
      const formatInfo = {
        ...
        totalFormats: SUPPORTED_FORMATS.length
      };
      ...
      return {
        content: [
          {
            type: 'text',
            text: `Error listing formats: ${(error as Error).message}`
          }
        ],
        isError: true
      };
